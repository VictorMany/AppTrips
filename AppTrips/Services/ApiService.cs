...
namespace AppTrips.Services
{
    ...
    {
        private string ApiUrl = "http://192.168.2.2/WebApiTrips/trips";
        ...
        public async Task<ApiResponse> DeleteDataAsync(string controller, TripModel viaje)
        {
            try
            {
                var client = new HttpClient
                ...
                var response = await client.DeleteAsync(controller + "/" + viaje.ID);
                var result = await response.Content.ReadAsStringAsync();
